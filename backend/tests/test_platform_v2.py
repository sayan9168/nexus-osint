...
def test_auth_round_trip():
    username="test_"+uuid4().hex[:10];user=create_user(username,"strong-password-123")
...
def test_entity_fingerprint_is_stable():assert fingerprint("domain","Example.COM.")==fingerprint("domain","example.com")
def test_relationship_merge_is_idempotent():
    case=store.create("relationship-test","").id
    rid1=merge_relationship(case,"a","b","resolves-to",.8)["id"];rid2=merge_relationship(case,"a","b","resolves-to",.9)["id"];assert rid1==rid2

def test_audit_chain_verifies():record("test","pytest","platform",{"ok":True});assert verify_chain()["valid"] is True
