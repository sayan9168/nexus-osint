...
router=APIRouter()
class CreateCase(BaseModel):name:str=Field(min_length=1,max_length=120);description:str=Field(default="",max_length=2000)
...
def create_case(payload:CreateCase,user=Depends(require_permission("case:write"))):
 c=store.create(payload.name,payload.description);record("case.create",user["username"],c.id,{"name":c.name});return c
...
def add_target(case_id:str,payload:TargetInput,user=Depends(require_permission("case:write"))):
 c=store.add_target(case_id,payload.target.strip())
 if not c:raise HTTPException(404,"Case not found")
 record("case.target.add",user["username"],case_id,{"target":payload.target.strip()});return c
