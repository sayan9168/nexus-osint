...
class JobRequest(BaseModel):
    case_id:str; target:str=Field(min_length=1,max_length=2048); target_type:EntityType; authorized:bool=False
@router.get("/plugins")
...
def create_job(request:JobRequest,user=Depends(require_permission("job:run"))):
    ...
    job=enqueue(request.case_id,request.target,request.target_type); record("job.enqueue",user["username"],request.case_id,request.target); return job
@router.get("/jobs")
