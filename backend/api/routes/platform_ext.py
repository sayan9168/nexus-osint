...
@router.get("/plugins")
def plugins(user=Depends(require_permission("case:read"))):
    return {"plugins": list_plugins()}

@router.get("/plugins/health")
def plugin_health(user=Depends(require_permission("case:read"))):
    return {"plugins": health()}

@router.get("/rbac/{role}")
def rbac(role: str, user=Depends(require_permission("case:read"))):
...
def create_job(request: JobRequest, user=Depends(require_permission("job:run"))):
    ...
    record("job.enqueue", user["username"], request.case_id, {"target": request.target, "target_type": request.target_type.value})
    return job

@router.get("/jobs")
def jobs(user=Depends(require_permission("case:read"))):
    return {"jobs": list_jobs()}

@router.get("/jobs/{job_id}")
def job(job_id: str, user=Depends(require_permission("case:read"))):
...
def cancel_job(job_id: str, user=Depends(require_permission("job:run"))):
    ...
    if not result:
        raise HTTPException(404, "Job not found")
    record("job.cancel", user["username"], job_id, {})
    return result

@router.get("/audit")
def audit(limit: int = 100, user=Depends(require_permission("audit:read"))):
    return {"events": list_events(limit)}

@router.get("/audit/verify")
def audit_verify(user=Depends(require_permission("audit:read"))):
    return verify_chain()
