...
async def check_account(account: str) -> dict:
    ...
    if not key:
        return {
            ...
            "account": account,
            "matches": [],
            "message": "Set HIBP_API_KEY to enable breach metadata checks. No leaked database is stored or downloaded.",
            "source": "Have I Been Pwned",
        }
    headers = {
        "hibp-api-key": key,
        "user-agent": "NEXUS-OSINT/4.0 breach-exposure",
        "accept": "application/json",
    }
    url = f"{HIBP_URL}/{httpx.URL(account).raw_path.decode('utf-8', errors='ignore')}"
    # URL-encode the account using httpx query machinery rather than interpolating
    # untrusted path content into a URL.
    ...
    try:
        ...
        if response.status_code == 404:
            return {"status": "no_known_breaches", "account": account, "matches": [], "source": "Have I Been Pwned"}
        if response.status_code == 429:
            return {"status": "rate_limited", "account": account, "matches": [], "source": "Have I Been Pwned"}
        if response.status_code in (401, 403):
            return {"status": "provider_auth_error", "account": account, "matches": [], "source": "Have I Been Pwned"}
