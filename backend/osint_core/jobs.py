...
def _collect_with_timeout(target,target_type):
 future=_collect_executor.submit(collect,target,target_type);return future.result(timeout=JOB_TIMEOUT_SECONDS)
def _run(job_id,case_id,target,target_type):
 current=_row(job_id)
 if not current or current["status"]=="cancelled": return
 attempt=max(1,int(current.get("attempts") or 0));_set(job_id,status="running",started_at=_now(),attempts=attempt,error=None)
 try:
 ...
 except FutureTimeout: _retry_or_fail(job_id,case_id,target,target_type,"job timeout")
 except Exception as exc: _retry_or_fail(job_id,case_id,target,target_type,f"{type(exc).__name__}: {exc}")
def _retry_or_fail(job_id,case_id,target,target_type,error):
 current=_row(job_id)
 if not current or current["status"]=="cancelled": return
 attempt=int(current.get("attempts") or 0)
 if attempt<MAX_ATTEMPTS:
  next_attempt=attempt+1;_set(job_id,status="queued",attempts=next_attempt,error=f"retry {next_attempt}/{MAX_ATTEMPTS}: {error}");_worker_executor.submit(_run,job_id,case_id,target,target_type)
 else: _set(job_id,status="failed",error=error,finished_at=_now())
def _celery_task():
 try:
  ...
  def run_job(self,job_id,case_id,target,target_type): _run(job_id,case_id,target,EntityType(target_type))
  return app,run_job
 except ImportError: return None,None
celery_app,_celery_run=_celery_task()
def recover_stale_jobs():
 for row in db.execute("SELECT id FROM jobs WHERE status='running'"): _set(row["id"],status="queued",error="recovered after worker restart")
 if os.getenv("NEXUS_USE_CELERY","false").lower()!="true":
  for row in db.execute("SELECT id,case_id,target,target_type FROM jobs WHERE status='queued' AND attempts<?",(MAX_ATTEMPTS,)): _worker_executor.submit(_run,row["id"],row["case_id"],row["target"],EntityType(row["target_type"]))
def enqueue(case_id,target,target_type):
 case=store.get(case_id)
 if not case: raise ValueError("case not found")
 if not case.authorized: raise ValueError("case is not authorized for collection")
 normalized=normalize_target(target,target_type);job_id=str(uuid4())
 ...
 else: _worker_executor.submit(_run,job_id,case_id,normalized,target_type)
 return _row(job_id)
def cancel(job_id):
 job=_row(job_id)
