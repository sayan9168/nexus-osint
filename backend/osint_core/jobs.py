...
_executor = ThreadPoolExecutor(max_workers=MAX_WORKERS, thread_name_prefix="nexus-osint")
...
def _collect_with_timeout(target, target_type):
    future = _executor.submit(collect, target, target_type)
...
def _celery_task():
    try:
        ...
        app.conf.update(
            ...
            task_soft_time_limit=max(5, JOB_TIMEOUT_SECONDS - 30),
...
def recover_stale_jobs():
    """Move interrupted queued/running jobs back to queued after a process restart."""
    ...
    if os.getenv("NEXUS_USE_CELERY", "false").lower() != "true":
        for row in db.execute("SELECT id,case_id,target,target_type FROM jobs WHERE status='queued' AND attempts<?", (MAX_ATTEMPTS,)):
            _executor.submit(_run, row["id"], row["case_id"], row["target"], EntityType(row["target_type"]))
...
def enqueue(case_id, target, target_type):
    ...
    job_id = str(uuid4())
    db.insert(
        "INSERT INTO jobs(id,case_id,target,target_type,status,created_at,attempts) VALUES(?,?,?,?,?,?,?)",
        (job_id, case_id, normalized, target_type.value, "queued", _now(), 1),
    )
