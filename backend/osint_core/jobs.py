...
def _set(job_id,**changes):
    ...
    fields=", ".join(f"{k}=?" for k in changes); db.insert(f"UPDATE jobs SET {fields} WHERE id=?",tuple(changes.values())+(job_id,))
...
def _run(job_id,case_id,target,target_type):
    ...
    try:
        ...
        if _row(job_id) and _row(job_id)["status"]=="cancelled": return
        for item in evidence: store.add_evidence(case_id,item)
        _set(job_id,status="completed",finished_at=_now())
    except Exception as exc:
        current=_row(job_id)
        if current and current["attempts"]<3 and current["status"]!="cancelled":
            _set(job_id,status="queued",attempts=current["attempts"]+1,error=f"retry: {type(exc).__name__}")
            _executor.submit(_run,job_id,case_id,target,target_type)
        else: _set(job_id,status="failed",error=f"{type(exc).__name__}: {exc}",finished_at=_now())
...
def _celery_task():
    try:
        ...
        app.conf.update(task_acks_late=True,worker_prefetch_multiplier=1,task_time_limit=300,task_soft_time_limit=270)
...
_celery,_celery_run=_celery_task()
def enqueue(case_id,target,target_type):
    if not store.get(case_id): raise ValueError("case not found")
    normalized=normalize_target(target,target_type); job_id=str(uuid4()); now=_now()
    db.insert("INSERT INTO jobs(id,case_id,target,target_type,status,created_at,attempts) VALUES(?,?,?,?,?,?,0)",(job_id,case_id,normalized,target_type.value,"queued",now))
    if os.getenv("NEXUS_USE_CELERY","false").lower()=="true" and _celery_run:
        try:_celery_run.delay(job_id,case_id,normalized,target_type.value)
...
def cancel(job_id):
    ...
    if _celery and _celery_run:
        try:_celery.control.revoke(job_id,terminate=False)
