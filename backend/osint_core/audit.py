...
def record(action:str,actor:str,resource:str="",metadata=None)->dict:
 now=datetime.now(timezone.utc).isoformat();event_id=str(uuid4());meta=metadata if isinstance(metadata,str) else json_dumps(metadata or {})
 previous=db.execute("SELECT event_hash FROM audit_events ORDER BY created_at DESC,id DESC LIMIT 1");prev=previous[0]["event_hash"] if previous else "GENESIS"
...
def list_events(limit:int=100):
 limit=max(1,min(limit,500));return [dict(r) for r in db.execute("SELECT * FROM audit_events ORDER BY created_at DESC,id DESC LIMIT ?",(limit,))]
def verify_chain()->dict:
 rows=db.execute("SELECT * FROM audit_events ORDER BY created_at ASC,id ASC");previous="GENESIS"
 for row in rows:
  expected=hashlib.sha256(f"{previous}|{row['id']}|{row['created_at']}|{row['actor']}|{row['action']}|{row['resource']}|{row['metadata']}".encode()).hexdigest()
  if row["previous_hash"]!=previous or row["event_hash"]!=expected:return {"valid":False,"broken_event":row["id"]}
